...
const mean = (town: string, strng: string): number => {
  const townData = strng
    .split('\n')
    .find((line) => line.startsWith(town + ':'));
  if (!townData) return -1; // Devuelve -1 si la ciudad no está en los datos
...
const variance = (town: string, strng: string): number => {
  const townData = strng
    .split('\n')
    .find((line) => line.startsWith(town + ':'));
  if (!townData) return -1; // Devuelve -1 si la ciudad no está en los datos
  ...
  return (
    records.reduce((a, b) => a + Math.pow(b - meanValue, 2), 0) / records.length
  );
};
...
console.log(mean('NonExistentTown', data));
console.log(variance('NonExistentTown', data));
