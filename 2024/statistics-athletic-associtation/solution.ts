function stat(s: string): string {
  ...
  const rangeInSeconds =
    Math.max(...timesInSeconds) - Math.min(...timesInSeconds);
  ...
  let medianInSeconds: number;
  const middle = Math.floor(sortedTimes.length / 2);
  if (sortedTimes.length % 2 === 0) {
    medianInSeconds = Math.floor(
  ...
  return `Range: ${toHHMMSS(rangeInSeconds)} Average: ${toHHMMSS(
    averageInSeconds
  )} Median: ${toHHMMSS(medianInSeconds)}`;
}
